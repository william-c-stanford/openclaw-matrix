...
fn test_gen_shade_color() {
    use super::gen_shade_color;
    use pretty_assertions::assert_eq;
    let black: [u8; 3] = [0, 0, 0];
    let green: [u8; 3] = [0, 255, 0];
    let length = 10;

    let colors = gen_shade_color(green, black, length);

    assert_eq!(colors.len(), length as usize);
    // First entry is exactly the base color
    assert_eq!(colors.first(), Some(&[0u8, 255, 0]));
    // Last entry is exactly the shade target
    assert_eq!(colors.last(), Some(&[0u8, 0, 0]));
}
